...
export default async function ContactDetailPage({
  ...
  return (
    <Shell
      crumb={contact.name}
      actions={
        <Link href="/app/contacts" className="btn btn-ghost btn-sm no-underline">
          ← All contacts
        </Link>
      }
    >
      <div className="mb-6 flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <div
            style={{ fontFamily: "var(--font-mono)", fontSize: ".78rem", color: "var(--muted)" }}
            className="mb-2 -mt-4"
          >
            {contact.email}
          </div>
      ...
      <div className="card mb-6">
        ...
        <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
          <div>
            <div
              className="mb-2"
              style={{ fontFamily: "var(--font-mono)", fontSize: ".6rem", letterSpacing: ".12em", color: "var(--faint)", textTransform: "uppercase" }}
            >
            ...
            <div
              ...
              style={{
                ...
                fontWeight: 700,
                letterSpacing: "-.03em",
                color: engagementColor(contact.engagement),
              }}
            >
            ...
            {features && (
              <table style={{ width: "100%" }}>
                <tbody>
                  ...
                  <FeatureRow
                    label="Last engaged"
                    value={features.lastEngagedDaysAgo === null ? "never" : `${features.lastEngagedDaysAgo}d ago`}
                  />
                </tbody>
              </table>
            )}
          ...
          <div>
            ...
            <div className="mb-3">
              <Pill tone={riskTone(contact.bounceRisk)}>{contact.bounceRisk}</Pill>
            </div>
            {features && (
              <table style={{ width: "100%" }}>
                <tbody>
                  ...
                  <FeatureRow
                    label="Specialization on file"
                    value={contact.specialization ?? "none"}
                  />
                </tbody>
              </table>
            )}
            ...
            {sends.map((s, i) => (
              <tr key={i} style={{ borderBottom: "1px solid var(--line)" }}>
                <td className="px-3.5 py-3">
                  <Link
                    href={`/app/campaigns/${s.campaignId}`}
                ...
                <td className="px-3.5 py-3">
                  <Pill tone={outcomeTone(s.outcome)}>{s.outcome}</Pill>
                </td>
              </tr>
            ))}
            {sends.length === 0 && (
              <tr>
                <td colSpan={3} className="px-3.5 py-8 text-center text-faint">
                  No sends recorded for this contact yet.
                </td>
              </tr>
            )}
          </tbody>
...
function FeatureRow({ label, value }: { label: string; value: string | number }) {
  return (
    <tr style={{ borderBottom: "1px solid var(--line)" }}>
      ...
      <td className="num py-2 text-right text-[.78rem]" style={{ color: "var(--color-paper)" }}>
        {value}
      </td>
    </tr>
  );
