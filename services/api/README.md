...
| [`routers/campaigns.py`](routers/campaigns.py) | `/api/organizations/{org_id}/campaigns` | Create, launch (the outbox-pattern fan-out — see its docstring for the exact commit/enqueue sequence), cancel, progress. |
...
- LLM-backed agents (`packages/shared/providers/llm.py` exists; nothing calls it yet —
  that's Phase 7).
- Redis rate limiting, OpenTelemetry tracing, in-app notifications backend.
